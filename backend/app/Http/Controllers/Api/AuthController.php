...
class AuthController extends Controller
{
    ...
    public function updateProfile(Request $request)
    {
        ...
        if ($request->has('photos') && is_array($request->photos)) {
            ...
            $newPhotoUrls = array_map(function ($p) {
                return is_string($p) ? $p : ($p['photo_url'] ?? $p['uri']);
            }, $validPhotos);

            $removedPhotoUrls = array_diff($existingPhotoUrls, $newPhotoUrls);
    ...
    public function uploadImage(Request $request)
    {
        ...
        if ($request->hasFile('image')) {
            ...
            // Amazon Rekognition NSFW Content Moderation
            $rekognition = app(\App\Services\AmazonRekognitionService::class);
            $moderationResult = $rekognition->detectModeration($savedFilePath);

            if ($moderationResult['is_nsfw']) {
                @unlink($savedFilePath);
                return response()->json([
                    'error' => 'NSFW_CONTENT_DETECTED',
                    'message' => $moderationResult['reason'] ?: 'Inappropriate content detected. Explicit or NSFW images are strictly prohibited on HeartLink.',
    ...
    private function checkIsNsfwImage(string $filePath): bool
    {
        ...
        // Downsample to 100x100 grid for fast performance
        $sampleW = 100;
        $sampleH = 100;
        ...
        for ($y = 0; $y < $sampleH; $y++) {
            for ($x = 0; $x < $sampleW; $x++) {
                ...
                // Skin color detection in RGB and YCbCr spaces
        ...
        // Flag NSFW if overall skin area > 42% or central region exposure > 50%
